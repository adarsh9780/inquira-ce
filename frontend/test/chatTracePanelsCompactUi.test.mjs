...
test('chat history renders subdued reasoning blocks before final response and keeps code details lightweight', () => {
  ...
  assert.equal(source.includes('class="thinking-block"'), true)
  assert.equal(source.includes('class="thinking-toggle"'), true)
  assert.equal(source.includes('class="thinking-static"'), true)
  assert.equal(source.includes('class="thinking-chevron"'), true)
