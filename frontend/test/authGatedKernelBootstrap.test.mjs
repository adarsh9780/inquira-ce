...
test('workspace kernel bootstrap in app store is gated by authentication state', () => {
  ...
  assert.equal(source.includes('async function ensureWorkspaceKernelConnected('), true)
  ...
  assert.equal(source.includes('void ensureWorkspaceKernelConnected(workspaceId)'), false)
})
