...
test('sidebar uses the unified compact workspace/chat layout', () => {
  ...
  assert.equal(source.includes('uiStore.setSidebarCollapsed(!uiStore.isSidebarCollapsed)'), true)
  ...
  assert.equal(source.includes("uiStore.isSidebarCollapsed ? 'max-w-0 opacity-0 ml-0'"), true)
