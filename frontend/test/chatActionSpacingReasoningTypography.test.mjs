...
test('chat action rows keep wider spacing and reasoning text matches final response size', () => {
  ...
  assert.equal(historySource.includes('class="space-y-4"'), true)
  assert.equal(historySource.includes('gap: 1rem;'), true)
  ...
  assert.equal(activitySource.includes('.tool-activity-detail-running {'), true)
