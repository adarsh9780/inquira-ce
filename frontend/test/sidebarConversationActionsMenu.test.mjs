...
test('sidebar conversation menu shows exact date, divider, rename, and delete', () => {
  ...
  assert.equal(menuSource.includes('<FloatingActionMenu'), true)
  assert.equal(menuSource.includes(':header="exactDate"'), true)
  assert.equal(menuSource.includes('dividerBefore'), true)
