...
test('table and figure tabs restore and persist selected artifacts through app store', () => {
  ...
  assert.equal(tableSource.includes('artifactApi.listTurn('), true)
  ...
  assert.equal(figureSource.includes('artifactApi.listTurn('), true)
  assert.equal(figureSource.includes('const liveFigureArtifacts = computed(() => {'), true)
})
