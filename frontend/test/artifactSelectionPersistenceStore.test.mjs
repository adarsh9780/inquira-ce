...
test('table and figure tabs restore and persist selected artifacts through app store', () => {
  ...
  assert.equal(figureSource.includes('appStore.getSelectedFigureArtifact(workspaceId)'), false)
  ...
  assert.equal(figureSource.includes('apiService.v1ListTurnArtifacts('), true)
  assert.equal(figureSource.includes('const liveFigurePayload = normalizePlotlyFigure(liveFigure?.data ?? liveFigure)'), false)
})
