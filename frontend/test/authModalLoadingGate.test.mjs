...
test('auth modal is not hidden behind authStore.isLoading gate', () => {
  ...
  assert.equal(source.includes(':is-open="isAuthUiReady && !authStore.isAuthenticated"'), true)
...
test('auth modal waits for backend readiness before it can open', () => {
  const source = readFileSync(
    resolve(process.cwd(), 'src/App.vue'),
    'utf-8',
  )
  ...
  assert.equal(source.includes('await authStore.checkAuth()'), true)
})
