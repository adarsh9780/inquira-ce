...
test('appearance tab exposes app and code font selectors wired to app store state', () => {
  ...
  assert.equal(source.includes('preferencesStore.setUiFont(fontId)'), true)
  ...
  assert.equal(source.includes('preferencesStore.setUiCodeFont(fontId)'), true)
...
test('app shell applies and persists font preference through font service', () => {
  ...
  assert.equal(source.includes('() => preferencesStore.uiFont'), true)
  assert.equal(source.includes('() => preferencesStore.uiCodeFont'), true)
  ...
  assert.equal(source.includes('preferencesStore.setUiFont(storedFont, { persist: false })'), true)
  assert.equal(source.includes('preferencesStore.setUiCodeFont(storedCodeFont, { persist: false })'), true)
})
...
test('style and store include offline-safe app and code font catalogs', () => {
  ...
  assert.equal(styleSource.includes('--font-ui: -apple-system, BlinkMacSystemFont'), true)
  ...
  assert.equal(fontsSource.includes("id: 'inter'"), true)
  assert.equal(fontsSource.includes("id: 'jetbrains-mono'"), true)
  assert.equal(fontsSource.includes("id: 'source-code-pro'"), true)
