...
test('turn mode keeps bounded history without legacy paging or composer navigation controls', () => {
  ...
  assert.equal(chatTabSource.includes('await conversationStore.fetchConversationTurns()'), true)
  ...
  assert.equal(chatInputSource.includes('@click="conversationStore.goToPreviousTurn()"'), false)
  assert.equal(chatInputSource.includes('@click="conversationStore.goToNextTurn()"'), false)
