...
test('sidebar uses native title tooltips for current navigation actions', () => {
  ...
  const conversationsSource = readFileSync(resolve(process.cwd(), 'src/components/layout/sidebar/SidebarConversations.vue'), 'utf-8')

  assert.equal(conversationsSource.includes('title="New Conversation"'), true)
