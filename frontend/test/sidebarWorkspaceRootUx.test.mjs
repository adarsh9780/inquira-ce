...
test('sidebar uses a minimal workspace dropdown above the explorer sections', () => {
  ...
  assert.equal(unifiedSource.includes('refreshWorkspaceDatasetSummary'), false)
  assert.equal(unifiedSource.includes("apiService.v1ListDatasets(workspaceId)"), false)
  assert.equal(unifiedSource.includes('filteredSidebarWorkspaces'), true)
  assert.equal(unifiedSource.includes('Search conversations'), false)
})
