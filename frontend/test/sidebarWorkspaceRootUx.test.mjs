...
test('sidebar keeps active workspace in root row and hides it from workspace list', () => {
  ...
  assert.equal(unifiedSource.includes('Active Workspace'), true)
  assert.equal(unifiedSource.includes('activeWorkspaceName'), true)
  assert.equal(unifiedSource.includes('v-else key="workspace-tree-detached"'), true)
  assert.equal(unifiedSource.includes('v-else-if="!appStore.isSidebarCollapsed"'), false)
  ...
  assert.equal(workspacesSource.includes('No other workspaces'), false)
})
