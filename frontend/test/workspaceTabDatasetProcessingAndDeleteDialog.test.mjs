...
test('workspace tab shows inline dataset processing card and consumes websocket progress', () => {
  ...
  assert.equal(source.includes('onProgress: (job) => {'), true)
  assert.equal(source.includes('onComplete: async (job) => {'), true)
  assert.equal(source.includes("setWorkspaceOperation('ingest', 'Importing selected datasets into the workspace.')"), false)
