...
test('chat input consumes live token events and does not fall back to non-stream analyze route', () => {
  ...
  assert.equal(source.includes('conversationStore.appendLastMessageExplanationChunk(evt.data.text, localMessageId, { conversationId: requestConversationId })'), true)
  assert.equal(source.includes("conversationStore.appendLastMessagePlanChunk(evt.data.text, evt.data.node || '', localMessageId, { conversationId: requestConversationId })"), true)
  assert.equal(source.includes('conversationStore.appendLastMessageTraceEvent({'), true)
