...
test('chat input consumes live token events and does not fall back to non-stream analyze route', () => {
  ...
  assert.equal(source.includes('appStore.appendLastMessageExplanationChunk(evt.data.text, localMessageId)'), true)
  assert.equal(source.includes("appStore.appendLastMessagePlanChunk(evt.data.text, evt.data.node || '', localMessageId)"), true)
