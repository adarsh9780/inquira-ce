...
test('workspace right pane routes third icon tab to output panel', () => {
  ...
  assert.equal(source.includes('class="workspace-toolbar-shell flex-shrink-0 h-12 px-3 flex items-center"'), true)
  assert.equal(source.includes('data-pane-tab-active'), true)
  assert.equal(source.includes(':aria-pressed="appStore.dataPane === \'output\'"'), true)
