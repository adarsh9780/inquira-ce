...
test('frontend API client exposes provider model search endpoint', () => {
  ...
  assert.equal(contract.includes("searchModels: (params) =>"), true)
  assert.equal(contract.includes("axios.get('/api/v1/preferences/models/search', { params })"), true)
  ...
  assert.equal(source.includes("q: query,"), true)
