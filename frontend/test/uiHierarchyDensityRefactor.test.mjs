...
test('chat keeps composer pinned as footer and improves assistant readability', () => {
  ...
  assert.equal(chatHistorySource.includes('class="ephemeral-trace-action"'), true)
  assert.equal(chatHistorySource.includes('class="ephemeral-trace-detail"'), true)
  assert.equal(chatHistorySource.includes('class="thinking-block"'), false)
  assert.equal(chatHistorySource.includes('&lt;/&gt; View code'), false)
...
test('table styling increases density and adds row/column tracking cues', () => {
  ...
  assert.equal(source.includes('--ag-even-row-background-color: var(--color-surface);'), true)
  assert.equal(source.includes('--ag-row-hover-color: var(--color-base-muted);'), true)
  assert.equal(source.includes('--ag-header-background-color: var(--color-surface);'), true)
  assert.equal(source.includes('--ag-active-color: var(--color-accent);'), true)
