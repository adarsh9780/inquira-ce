...
test('command palette is app-level, uses Cmd/Ctrl+K, and launches common commands', () => {
  ...
  assert.equal(app.includes('uiStore.toggleCommandPalette()'), true)
  ...
  assert.equal(leftPane.includes('appStore.openCommandPalette()'), false)
  assert.equal(leftPane.includes('MagnifyingGlassIcon'), false)
  ...
  assert.equal(modal.includes("uiStore.setSidebarCollapsed(!uiStore.isSidebarCollapsed)"), true)
  assert.equal(modal.includes('uiStore.openKeyboardShortcuts()'), true)
  assert.equal(modal.includes('appStore.openDataConnectionFlow()'), true)
