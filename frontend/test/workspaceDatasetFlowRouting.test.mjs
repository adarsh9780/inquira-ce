...
test('unified sidebar routes workspace and dataset entry points to settings workspace tab with proper steps', () => {
  const source = readFileSync(resolve(process.cwd(), 'src/components/layout/UnifiedSidebar.vue'), 'utf-8')

  assert.equal(source.includes("@click.stop=\"openSettings('workspace', 2)\""), true)
