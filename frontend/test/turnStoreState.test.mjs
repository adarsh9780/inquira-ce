...
test('app store exposes active turn state and loaders', () => {
  ...
  const source = `${readFileSync(resolve(testDir, '../src/stores/conversationStore.ts'), 'utf-8')}\n${readFileSync(resolve(testDir, '../src/stores/appCoordinatorStore.js'), 'utf-8')}`
  ...
  assert.match(source, /const activeTurnArtifacts = ref<.*>\(\[\]\)/)
  assert.match(source, /const activeTurnRelations = ref<.*>\(null\)/)
  assert.match(source, /const activeTurnTree = ref<.*>\(null\)/)
  ...
  assert.equal(source.includes('await loadActiveTurnTree(conversationId, targetTurnId)'), true)
  assert.equal(source.includes('async function loadActiveTurnTree('), true)
