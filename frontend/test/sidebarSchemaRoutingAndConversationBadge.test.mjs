...
test('sidebar adds workspace view entries and uses numbered conversation badges', () => {
  ...
  assert.equal(sidebarSource.includes('min-h-0 flex-1 overflow-hidden pb-1'), true)
  ...
  assert.equal(sidebarSource.includes("appStore.isSidebarCollapsed ? 'justify-center px-0 py-1.5' : 'justify-start px-3 py-1.5'"), false)
  assert.equal(sidebarSource.includes('conversationBadgeLabel(index, appStore.conversations.length)'), false)
  assert.equal(sidebarSource.includes('const ordinal = total - offset'), false)
  assert.equal(sidebarSource.includes("if (ordinal > 99) return '99+'"), false)
