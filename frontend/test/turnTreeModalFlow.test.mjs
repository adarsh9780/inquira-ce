...
test('workspace turn tree flow loads the full tree and restores state from selection', () => {
  ...
  const apiSource = readFileSync(resolve(testDir, '../src/api/conversations.ts'), 'utf-8')
  ...
  assert.equal(apiSource.includes('async workspaceTurnTree(workspaceId'), true)
  assert.equal(apiSource.includes("invokeNative<unknown[]>('ListConversationTurns'"), true)
  assert.equal(apiSource.includes('const tree = nativeTurnTree('), true)
  assert.equal(apiSource.includes('getTurnTree'), false)
  assert.equal(storeSource.includes('function setWorkspaceTurnTree(payload)'), true)
  assert.equal(storeSource.includes('conversationApi.workspaceTurnTree(targetWorkspaceId)'), true)
  ...
  assert.equal(treeActionsSource.includes('await conversationApi.getTurn(conversationId, turnId)'), true)
