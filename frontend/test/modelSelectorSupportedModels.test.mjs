...
test('model selector accepts injected model options prop', () => {
  ...
  assert.equal(source.includes('const availableModels = computed<ModelOption[]>(() => {'), true)
  assert.equal(source.includes('<HeaderDropdown'), true)
  assert.equal(source.includes('search-placeholder="Search model"'), true)
  assert.equal(source.includes(':max-options-without-search="maxOptionsWithoutSearch"'), true)
  assert.equal(source.includes('const source = props.modelOptions.length ? props.modelOptions : fallbackModels'), true)
  ...
  assert.equal(source.includes(':trigger-label="selectedLabel"'), true)
})
