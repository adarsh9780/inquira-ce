...
test('desktop releases build only after a stable release is published', () => {
  ...
  assert.match(workflow, /version="\$\(tr -d '\[:space:\]' < VERSION\)"/)
  assert.match(workflow, /Release tag \$tag does not match canonical VERSION \$version/)
  assert.match(workflow, /go run \.\/cmd\/prepareversion/)
  assert.doesNotMatch(workflow, /cmd\/releaseprep/)
  assert.doesNotMatch(workflow, /process\.env\.INQUIRA_BUILD_VERSION/)
