...
test('code tab runs manual code through execution service and surfaces output in the results pane', () => {
  ...
  assert.equal(source.includes('await executeSnippet(executionStore.pythonFileContent, \'Code executed successfully!\''), true)
  assert.equal(source.includes("uiStore.setActiveTab('output')"), true)
  assert.equal(source.includes("executionStore.setTerminalOutput('Running code...')"), true)
...
test('chat tab relies on sidebar lifecycle controls for usable analysis sessions', () => {
  ...
  assert.equal(source.includes('await conversationStore.fetchConversationTurns()'), true)
