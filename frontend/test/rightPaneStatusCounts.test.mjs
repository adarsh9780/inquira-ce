...
test('workspace right pane supports one three-option category selector and lazy renderers', () => {
  ...
  assert.equal(source.includes('resultCategoryOptions = computed(() => ['), true)
  ...
  assert.equal(source.includes("{ value: 'table', label: 'Tables', icon: TableCellsIcon, count: tableResultCount.value }"), true)
  assert.equal(source.includes("{ value: 'chart', label: 'Charts', icon: ChartBarIcon, count: chartResultCount.value }"), true)
