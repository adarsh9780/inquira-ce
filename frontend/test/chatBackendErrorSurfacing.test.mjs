...
test('chat input surfaces backend error detail instead of flattening 5xx into generic copy', () => {
  const source = [
    readFileSync(resolve(process.cwd(), 'src/components/chat/ChatInput.vue'), 'utf-8'),
  ...
  assert.equal(source.includes("if (event.event === 'error')"), true)
  assert.equal(source.includes('conversations.updateLastMessageExplanation('), true)
})
