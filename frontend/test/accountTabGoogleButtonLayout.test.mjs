...
test('settings account tab keeps Google auth controls and local-user fallback messaging', () => {
  ...
  assert.equal(source.includes('Google account linked.'), true)
  assert.equal(source.includes('Signing out from Google converts the account back to Local User mode.'), true)
