...
test('markdown code fences render editor-like block with copy icon and delegated copy handler', () => {
  ...
  const prismLanguages = readFileSync(resolve(process.cwd(), 'src/utils/prismLanguages.ts'), 'utf-8')
  const markdownContent = readFileSync(resolve(process.cwd(), 'src/components/chat/MarkdownContent.vue'), 'utf-8')
  const source = [history, renderer, prismLanguages, markdownContent].join('\n')
  ...
  assert.equal(source.includes('Prism.languages.python ='), true)
  assert.equal(source.includes('Prism.languages.sql ='), true)
