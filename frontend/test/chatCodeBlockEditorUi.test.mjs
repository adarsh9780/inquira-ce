...
test('markdown code fences render editor-like block with copy icon and delegated copy handler', () => {
  ...
  const scrollLifecycleSource = readFileSync(
    resolve(process.cwd(), 'src/composables/useChatScrollFollow.js'),
    'utf-8',
  )
  ...
  assert.equal(source.includes('onContainerClick: handleChatContainerClick'), true)
  assert.equal(scrollLifecycleSource.includes("container.addEventListener('click', onContainerClick)"), true)
  ...
  assert.equal(source.includes('function renderCodeSnapshot(code)'), true)
