...
test('workspace detail shows runtime recovery actions and inspect-first activation flow', () => {
  ...
  assert.equal(source.includes('Runtime activity'), true)
  ...
  assert.equal(source.includes('class="flex items-start justify-between gap-3 border-b border-[var(--color-border)] pb-4"'), true)
  assert.equal(source.includes('divide-y divide-[var(--color-border)]'), true)
