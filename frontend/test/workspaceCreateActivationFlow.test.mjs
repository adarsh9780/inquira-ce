...
test('workspace setup stepper captures shared context before dataset selection', () => {
  ...
  assert.equal(source.includes('await waitForCreateRuntimeToSettle(workspaceId)'), true)
  assert.equal(source.includes("toast.success('Workspace created', 'Workspace is ready for dataset selection.')"), true)
  assert.equal(source.includes('settingsWebSocket.subscribeComplete(handleSettingsComplete)'), true)
  assert.equal(source.includes("const emit = defineEmits(['navigate', 'set-active-workspace', 'creation-lock-change'])"), true)
  assert.equal(source.includes("emit('creation-lock-change', Boolean(locked))"), true)
  assert.equal(settingsSource.includes('const workspaceCreationLocked = ref(false)'), true)
  assert.equal(settingsSource.includes(':disabled="workspaceCreationLocked"'), true)
  assert.equal(settingsSource.includes('@creation-lock-change="setWorkspaceCreationLocked"'), true)
  assert.equal(settingsSource.includes('if (workspaceCreationLocked.value) return'), true)
