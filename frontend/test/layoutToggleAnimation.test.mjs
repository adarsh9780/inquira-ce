...
test('sidebar icons keep fixed sizing during collapse/expand', () => {
  ...
  assert.equal(sidebarSource.includes('Cog6ToothIcon class="h-4 w-4"'), true)
  assert.equal(sidebarSource.includes('ChevronDoubleRightIcon'), true)
  assert.equal(sidebarSource.includes('ChevronDoubleLeftIcon v-else class="h-4 w-4"'), true)
