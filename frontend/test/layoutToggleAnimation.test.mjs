...
test('sidebar keeps the current animated text-collapse and scroll layout', () => {
  ...
  assert.equal(sidebarSource.includes('class="max-h-[11.75rem] flex-none overflow-y-auto overflow-x-hidden pb-1"'), true)
  assert.equal(sidebarSource.includes('class="group relative flex min-h-9 select-none items-center rounded-lg cursor-pointer transition-colors hover:bg-[var(--color-text-main)]/5"'), true)
