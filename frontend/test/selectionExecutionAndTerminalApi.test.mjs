...
test('code tab binds Shift+Enter to selected code execution', () => {
  ...
  assert.equal(source.includes('preserveActiveTabOnNoOutput: true'), true)
  assert.equal(source.includes('Selection-style no-op execution (e.g. assignment only): keep current pane.'), true)
  assert.equal(source.includes("appStore.setActiveTab('output')"), true)
  assert.equal(source.includes('function startRunEntry(scopeLabel, code, { selectResult = false } = {}) {'), true)
  ...
  assert.equal(source.includes('selectResult: true'), true)
