...
test('settings modal uses v-model contract and two-column panel navigation', () => {
  ...
  assert.equal(source.includes('const navLevel = ref(1)'), true)
  ...
  assert.equal(source.includes('openWorkspaceLevel'), true)
  assert.equal(source.includes('closeWorkspaceLevel'), true)
...
test('workspace tab now uses list/detail/create panels instead of stepper flow', () => {
  ...
  assert.equal(tabSource.includes('Remove this dataset?'), true)
