...
test('settings modal uses v-model contract and two-column panel navigation', () => {
  ...
  assert.equal(source.includes('w-44'), true)
  assert.equal(source.includes('max-w-[70rem]'), true)
  assert.equal(source.includes('h-[min(47.5rem,calc(100dvh-2rem))]'), true)
...
test('settings modal selects its requested panel before loading workspace data', () => {
  const source = read('src/components/modals/SettingsModal.vue')
  const initializeIndex = source.indexOf('initializePanelState(props.initialTab)')
  const fetchIndex = source.indexOf('await workspaceStore.fetchWorkspaces()')
...
test('workspace tab separates focused creation from the unified management surface', () => {
  ...
  assert.equal(tabSource.includes('Add your first data source'), true)
  assert.equal(tabSource.includes('No data sources yet'), false)
  assert.equal(tabSource.includes('data-testid="workspace-import-datasets-dropzone"'), false)
