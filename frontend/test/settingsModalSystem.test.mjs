...
test('workspace tab uses a unified summary and editor instead of panel or step routing', () => {
  const tabSource = read('src/components/modals/tabs/WorkspaceTab.vue')

  assert.equal(tabSource.includes("const workspaceSurface = ref('summary')"), true)
  assert.equal(tabSource.includes("workspaceSurface === 'summary'"), true)
  assert.equal(tabSource.includes('New workspace'), true)
  assert.equal(tabSource.includes('@click="beginInlineCreate"'), true)
  assert.equal(tabSource.includes('@click="returnToWorkspaceSummary"'), true)
