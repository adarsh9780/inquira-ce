...
export type NormalizedUsage = Record<UsageField, number | null>
...
export function toProvidedUsageNumber(value: unknown): number | null {
...
export function normalizeUsage(value: unknown): NormalizedUsage | null {
  ...
  for (const field of USAGE_FIELDS) {
    const parsed = toProvidedUsageNumber(record[field])
...
export function mergeUsageTotals(base: unknown, incoming: unknown): NormalizedUsage | null {
...
export function formatUsageCompact(value: unknown): string {
...
export function formatUsageTooltip(value: unknown, summary: unknown = null): string {
  const usage = normalizeUsage(value) || emptyUsage()
  const lines = [
  ...
  const turnCount = Number(summaryRecord?.turn_count)
  const turnsWithUsage = Number(summaryRecord?.turns_with_usage)
