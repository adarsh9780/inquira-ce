...
export interface TurnTreeGraphNode extends RecordValue {
  id: string
  display_no: number
  x: number
  y: number
...
export interface TurnTreeGraphEdge {
  id: string
  parentId: string
  childId: string
}

export interface TurnTreeGraphBounds {
  x: number
  y: number
  width: number
  height: number
...
export interface TurnTreeGraphLayout {
  nodes: TurnTreeGraphNode[]
  edges: TurnTreeGraphEdge[]
  bounds: TurnTreeGraphBounds
}

export interface TurnTreeGraphPosition {
  x: number
  y: number
}

function asNode(value: unknown): RecordValue | null {
  return value !== null && typeof value === 'object' && !Array.isArray(value)
    ? value as RecordValue
    : null
}

export function layoutTurnTree(roots: unknown): TurnTreeGraphLayout {
  const nodes: TurnTreeGraphNode[] = []
  const edges: TurnTreeGraphEdge[] = []
  const seenIds = new Set<string>()
  const displayNumbers = new Map<string, number>()
  let nextLeafY = TURN_TREE_GRAPH_PADDING

  const visibleNodes: RecordValue[] = []
  function collectVisibleNodes(rawNodes: unknown) {
    for (const value of Array.isArray(rawNodes) ? rawNodes : []) {
      const rawNode = asNode(value)
      if (!rawNode) continue
  ...
  function visit(
    ...
    const childLayouts: TurnTreeGraphNode[] = []
    ...
    const node: TurnTreeGraphNode = {
      ...rawNode,
      id,
      display_no: displayNumbers.get(id) || 0,
...
export function turnTreeGraphEdgePath(
  parent: TurnTreeGraphPosition | null | undefined,
  child: TurnTreeGraphPosition | null | undefined,
): string {
...
export function turnTreeGraphPort(
  node: TurnTreeGraphPosition | null | undefined,
  side: 'input' | 'output',
): TurnTreeGraphPosition {
