export const dropdownSurfaceClass = 'motion-popover-surface layer-modal-dropdown overflow-hidden rounded-md py-1 shadow-md focus:outline-none'
...
interface DropdownSurfaceOptions {
  surface?: string
  border?: string
}

interface DropdownOptionStyleOptions {
  activeMix?: string
}

export function dropdownSurfaceStyle({
  surface = 'var(--color-surface)',
  border = 'var(--color-border)',
}: DropdownSurfaceOptions = {}): StyleRecord {
  return {
...
export function dropdownOptionStyle(
  active: boolean,
  { activeMix = 'var(--color-text-main) 6%' }: DropdownOptionStyleOptions = {},
): StyleRecord {
  return {
