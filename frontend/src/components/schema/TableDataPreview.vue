<template>
  <section class="flex min-h-[30rem] flex-1 flex-col overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm" :aria-labelledby="`preview-${table.id}-title`">
    <header class="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--color-border)] px-4 py-3">
      <div class="min-w-0">
        <h3 :id="`preview-${table.id}-title`" class="truncate font-mono text-[14px] font-semibold text-[var(--color-text-main)]">
          {{ table.tableName }}
        </h3>
