<template>
  <section class="flex h-full min-h-0 flex-col" aria-labelledby="workspace-sources-title">
    <header class="flex shrink-0 items-start justify-between gap-4 border-b border-[var(--color-border)] px-5 py-4">
      <div class="min-w-0">
        <p class="text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--color-text-muted)]">Workspace data</p>
        <h3 id="workspace-sources-title" class="mt-1 text-[14px] font-semibold text-[var(--color-text-main)]">Data sources</h3>
        <p class="mt-1 text-[12px] leading-5 text-[var(--color-text-muted)]">Add, refresh, or remove local CSV, Parquet, Excel, JSON, and SQLite snapshots.</p>
      </div>
      <button type="button" class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-[var(--color-text-muted)] transition-colors hover:bg-[var(--color-base-muted)] hover:text-[var(--color-text-main)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]" aria-label="Close data sources" @click="emit('close')">
    ...
    <div class="schema-scroll-area min-h-0 flex-1 overflow-y-auto px-5 pb-8">
      ...
      <p v-if="state.error.value" class="rounded-lg bg-[var(--color-danger-bg)] px-3 py-2 text-xs text-[var(--color-danger-text)]" role="alert">{{ state.error.value }}</p>
      ...
      <div v-if="isObjectSelectionAdapter(state.pending.value.adapterKind)" class="mt-4 space-y-2">
        <div class="flex items-center justify-between gap-3">
          ...
          <label v-if="state.pending.value.adapterKind === 'excel'" class="flex items-center gap-2 text-xs text-[var(--color-text-muted)]">
            Formula values
            <HeaderDropdown
              v-model="state.pending.value.formulaMode"
              :options="formulaModeOptions"
              aria-label="Excel formula values"
              max-width-class="w-40"
              :disabled="state.busy.value"
              @update:model-value="state.previewObject(state.pending.value?.activeObjectId)"
            />
          </label>
        </div>
...
const emit = defineEmits<{ changed: []; close: []; 'update:count': [count: number] }>()
...
const formulaModeOptions = [
  { value: 'cached', label: 'Last saved values' },
  { value: 'formula', label: 'Formula text' },
]
...
.schema-scroll-area { scrollbar-width: none; }
.schema-scroll-area::-webkit-scrollbar { width: 0; height: 0; }
</style>
