...
export const APP_FONT_OPTIONS: readonly FontOption[] = [
  {
    id: 'default',
    label: 'Default',
...
export function normalizeAppFontId(fontId: unknown): string {
...
export function normalizeCodeFontId(fontId: unknown): string {
  const normalized = String(fontId || '').trim().toLowerCase()
