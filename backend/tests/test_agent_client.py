...
@pytest.mark.asyncio
async def test_agent_client_stream_retries_once_on_retryable_provider_api_error(monkeypatch):
    class _Resp:
        def __init__(self, status_code: int = 200, payload=None, lines=None):
            self.status_code = status_code
            self.text = ""
    ...
    class _Client:
        ...
        def stream(self, method, url, *args, **kwargs):
            ...
            if type(self).stream_calls == 1:
                lines = [
                    "event: error",
                    'data: {"error":"APIError","message":"An internal error occurred"}',
                    "",
                ]
            else:
                lines = [
    ...
    assert _Client.stream_calls == 2
    assert [evt["event"] for evt in events] == ["values"]
