...
@pytest.mark.asyncio
async def test_regenerate_schema_preserves_upstream_http_status(monkeypatch, tmp_path):
    duckdb_path = tmp_path / "workspace.duckdb"
    duckdb_path.touch()
    ...
    assert "Failed to generate schema via LLM: Insufficient credits" in str(exc_info.value.detail)
