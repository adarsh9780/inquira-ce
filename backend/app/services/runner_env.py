...
def ensure_runner_kernel_dependencies(config: ExecutionRuntimeConfig) -> None:
    ...
    _install_with_uv_or_pip(runner_python, required_packages)
...
def _install_with_uv_or_pip(runner_python: str, packages: list[str]) -> None:
    uv_cmd = ["uv", "pip", "install", "--python", runner_python, *packages]
    uv_proc = subprocess.run(uv_cmd, capture_output=True, text=True)
    if uv_proc.returncode == 0:
        return

    pip_cmd = [runner_python, "-m", "pip", "install", *packages]
