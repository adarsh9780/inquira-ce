    ...
    for ds in datasets:
        ...
        response_datasets.append(
            DatasetResponse(
                ...
                file_size_mb=round(ds["file_size"] / (1024 * 1024), 2)
                if ds["file_size"]
                else 0.0,
...
def check_dataset_health(
    table_name: str, current_user: dict = Depends(get_current_user)
):
    ...
    if not ds:
        return DatasetHealthResponse(
            table_name=table_name, healthy=False, error="Dataset not found in catalog"
        )
    ...
    try:
        ...
        return DatasetHealthResponse(
            table_name=table_name, healthy=True, row_count=result[0] if result else 0
        )
    except Exception as e:
        return DatasetHealthResponse(table_name=table_name, healthy=False, error=str(e))
...
def delete_dataset_endpoint(
    ...
    debug_log(
        f"📋 Found dataset: file_path={ds.get('file_path')}, schema_path={ds.get('schema_path')}"
    )
    ...
    try:
        dataset_dir = get_dataset_dir(
            user_id, table_name, create=False
        )  # Don't create!
        ...
        if dataset_dir.exists():
            ...
            debug_log(
                f"📂 Directory contents before delete: {[str(c) for c in contents]}"
            )
        ...
        logprint(
            f"⚠️ [Datasets] Failed to delete dataset directory: {e}", level="warning"
        )
    ...
    try:
        from ..database.schema_storage import (
            get_schema_filename,
            get_legacy_schemas_dir as get_legacy_dir,
        )
    ...
    # 5. Delete associated chat sessions and LangGraph thread
    file_path = ds.get("file_path")
    if file_path:
        deleted_sessions = delete_sessions_for_data_path(user_id, file_path)
        debug_log(f"🗑️ Deleted {deleted_sessions} chat session(s) for file: {file_path}")
        if deleted_sessions > 0:
            deleted_files.append(f"Chat sessions: {deleted_sessions}")

        # Also delete LangGraph checkpointer data
        deleted_thread = delete_langgraph_thread(user_id, file_path)
        debug_log(
            f"🗑️ Deleted {deleted_thread} LangGraph checkpointer row(s) for file: {file_path}"
        )
        if deleted_thread > 0:
            deleted_files.append(f"LangGraph thread: {deleted_thread} rows")

    # 6. Delete from datasets catalog
    ...
    if success:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to delete dataset from catalog"
        )
...
def refresh_dataset_endpoint(
    ...
    try:
        ...
        if request.regenerate_schema:
            try:
                ...
                if api_key:
                    ...
                    logprint(
                        f"⚠️ [Datasets] No API key found, skipping schema regeneration",
                        level="warning",
                    )
            except Exception as schema_err:
                logprint(
                    f"⚠️ [Datasets] Schema regeneration failed: {schema_err}",
                    level="warning",
                )
        ...
        # RESTORE from backup
        logprint(
            f"❌ [Datasets] Refresh failed, restoring from backup: {e}", level="error"
        )
        try:
            conn.execute(f"DROP TABLE IF EXISTS {table_name}")
