...
@patch("agent_v2.nodes._invoke_structured_chain")
def test_chat_node_no_name_error(mock_invoke, mock_get_model, mock_emit_chunks) -> None:
    mock_get_model.return_value = MagicMock()
    # Mock ChatOutput structure
    mock_chat_output = MagicMock()
    mock_chat_output.answer = "Mock response"
    mock_invoke.return_value = mock_chat_output
