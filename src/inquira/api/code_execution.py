...
class CodeExecutionWithVariablesResponse(BaseModel):
    ...
    variables: dict = Field(
        default_factory=dict, description="Dictionary of variables created by the code"
    )
    output: Optional[str] = Field(None, description="Captured stdout output")
    error: Optional[str] = Field(None, description="Error message if execution failed")
    execution_time: float = Field(..., description="Time taken to execute the code")
...
async def execute_code(
    ...
    try:
        ...
        if (
            hasattr(app_state, "db_manager")
            and hasattr(app_state, "data_path")
            and app_state.data_path
        ):
            try:
                user_id = current_user["user_id"]
                db_connection = app_state.db_manager.get_connection(
                    user_id, app_state.data_path
                )
                # Inject the connection into CodeWhisperer as 'conn'
                code_whisperer.set_cached_connection("conn", db_connection)
                logprint(
                    f"DEBUG: Injected database connection as 'conn' for user {user_id}",
                    level="debug",
                )
            except Exception as e:
                logprint(
                    f"DEBUG: Could not inject database connection: {e}", level="debug"
                )

        # Execute the code with user session
        result, error = code_whisperer.execute_with_timeout(
            request.code, current_user["user_id"]
        )
        logprint(
            f"DEBUG: Execution result - Result: {result}, Error: {error}", level="debug"
        )
...
async def get_session_variables(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get session variables: {str(e)}"
        )
...
async def clear_session_variables(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return {
            "message": f"Session variables cleared for user {current_user['user_id']}"
        }

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to clear session variables: {str(e)}"
        )
...
async def execute_code_with_variables(
    ...
    try:
        ...
        result, variables, error = code_whisperer.execute_with_variables(
            request.code, current_user["user_id"]
        )
