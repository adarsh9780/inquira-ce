    ...
    async def handle(self, request: dict[str, Any], emit: Callable[[dict[str, Any]], Any]) -> dict[str, Any]:
        ...
        try:
            if not isinstance(request, dict) or not request_id or not isinstance(request.get("method"), str):
                raise RuntimeRequestError("invalid_request", "RPC request requires an id and method.")
            params = request.get("params")
            if not isinstance(params, dict):
                raise RuntimeRequestError("invalid_params", "RPC params must be an object.")
            ...
            if method == "ping":
                ...
                workspace_id = _workspace_id(params)
                response["result"] = {"workspace_id": workspace_id, "reset": await self.kernels.reset(workspace_id)}
            elif method == "kernel_interrupt":
                workspace_id = _workspace_id(params)
                response["result"] = {"workspace_id": workspace_id, "interrupted": await self.kernels.interrupt(workspace_id)}
            elif method == "kernel_prune":
                response["result"] = {"pruned": await self.kernels.prune_idle()}
            else:
                raise RuntimeRequestError("method_not_found", f"RPC method {method} was not found.")
        except RuntimeRequestError as exc:
            response["error"] = {"code": exc.code, "message": exc.message}
        except Exception as exc:
            response["error"] = {"code": "worker_internal_error", "message": str(exc) or "The worker could not complete the request."}
        return response
...
def _workspace_id(params: dict[str, Any]) -> str:
    ...
    if not isinstance(value, str) or not value.strip() or Path(value).name != value or "/" in value or "\\" in value:
...
def _execution_params(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(database_path, str) or not Path(database_path).expanduser().is_file():
        raise RuntimeRequestError("invalid_params", "database_path must identify an existing workspace catalog.")
    ...
    if not isinstance(artifact_dir, str) or not Path(artifact_dir).expanduser().is_absolute():
        raise RuntimeRequestError("invalid_params", "artifact_dir must be absolute.")
    if not isinstance(timeout, int) or timeout < 1 or timeout > 3600:
        raise RuntimeRequestError("invalid_params", "timeout_seconds must be between 1 and 3600.")
    return {
