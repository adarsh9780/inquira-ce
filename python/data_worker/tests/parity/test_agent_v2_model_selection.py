...
def test_get_model_prefers_lite_model_for_lite_tasks(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.load_llm_runtime_config", _runtime_stub)
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.provider_requires_api_key", lambda _provider: False)

    def fake_create_chat_model(**kwargs):
        captured.update(kwargs)
        return object()

    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.create_chat_model", fake_create_chat_model)
...
async def test_router_prefers_lite_model_for_route_decision(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.router.load_llm_runtime_config", _runtime_stub)
    monkeypatch.setattr("inquira_data_worker.agent_v2.router.ChatPromptTemplate.from_messages", lambda *_args, **_kwargs: _FakePrompt())

    def fake_create_chat_model(**kwargs):
        captured.update(kwargs)
        return _FakeModel()

    monkeypatch.setattr("inquira_data_worker.agent_v2.router.create_chat_model", fake_create_chat_model)
...
async def test_router_falls_back_structured_output_method_for_ollama(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.router.load_llm_runtime_config", _runtime_stub)
    monkeypatch.setattr("inquira_data_worker.agent_v2.router.ChatPromptTemplate.from_messages", lambda *_args, **_kwargs: _FakePrompt())
    monkeypatch.setattr("inquira_data_worker.agent_v2.router.create_chat_model", lambda **_kwargs: fake_model)
