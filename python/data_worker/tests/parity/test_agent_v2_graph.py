...
from inquira_data_worker.agent_v2.graph import _prepare_input_node, build_graph
...
def test_agent_v2_build_graph_initializes_phoenix_tracing(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.graph.init_phoenix_tracing", fake_init)
    _ = build_graph({})
...
def test_agent_v2_graph_contains_custom_tool_loop_for_context_enrichment() -> None:
    graph = build_graph({})
    rendered = graph.get_graph()
...
def test_agent_v2_graph_contains_runtime_custom_tool_routing() -> None:
    graph = build_graph({})
    rendered = graph.get_graph()
