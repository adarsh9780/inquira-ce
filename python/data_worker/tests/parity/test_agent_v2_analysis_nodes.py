...
from inquira_data_worker.agent_v2.coding_subagent.schema import AnalysisOutput
...
async def test_analysis_enrich_context_node_produces_structured_pending_tools(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._get_model", lambda *_args, **_kwargs: _FakeModel())
...
async def test_analysis_enrich_context_node_prefetches_schema_before_model_planning(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._get_model", fail_model)
    state = {
        "known_columns": [],
        "analysis_context": {
            "data_path": "",
...
async def test_analysis_enrich_context_node_skips_prefetch_when_context_pack_exists(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._ainvoke_provider_structured_chain", _fake_structured_chain)
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._get_model", lambda *_args, **_kwargs: object())
    state = {
        "known_columns": [],
        "analysis_context": {
            "data_path": "",
...
async def test_analysis_runtime_tools_node_emits_tool_messages_and_trace_hooks(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "inquira_data_worker.agent_v2.nodes.sample_data",
        lambda **_kwargs: {"rows": [{"customer_id": "c1"}], "columns": ["customer_id"], "row_count": 1},
    )
    monkeypatch.setattr(
        "inquira_data_worker.agent_v2.nodes.emit_agent_event",
        lambda event, payload: events.append((event, payload)),
    )
    monkeypatch.setattr(
        "inquira_data_worker.agent_v2.nodes._record_tool_span_event",
...
async def test_analysis_generate_code_requests_schema_enrichment_when_query_is_in_code(monkeypatch) -> None:
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.sample_data", lambda **_kwargs: [])
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._get_model", lambda *_args, **_kwargs: object())
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.build_coding_chain", lambda **_kwargs: object())
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.ainvoke_coding_chain", fake_ainvoke_coding_chain)
...
async def test_analysis_generate_code_falls_back_structured_output_method_for_ollama(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.build_coding_chain", fake_build_coding_chain)
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.ainvoke_coding_chain", fake_ainvoke_coding_chain)
...
async def test_analysis_generate_code_prefers_tool_call_path_for_ollama_cloud_models(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._get_model", lambda *_args, **_kwargs: _FakeModel())
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.sample_data", lambda **_kwargs: [])
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.build_coding_tool_call_chain", lambda **_kwargs: {"tool_chain": True})
    ...
    monkeypatch.setattr(
        "inquira_data_worker.agent_v2.nodes.ainvoke_coding_tool_call_chain",
...
async def test_analysis_generate_code_does_not_append_full_schema_memory_when_context_pack_exists(
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._get_model", lambda *_args, **_kwargs: object())
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.build_coding_chain", lambda **kwargs: {"method": kwargs.get("method")})
...
async def test_analysis_collect_context_adds_conversation_memory_summary(monkeypatch) -> None:
    monkeypatch.setattr(
        "inquira_data_worker.agent_v2.nodes.load_agent_runtime_config",
...
async def test_analysis_validate_result_retries_when_execution_has_no_signal(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.validate_and_summarize_result", fake_validate_and_summarize_result)
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._generate_result_explanations", fake_generate_result_explanations)
...
async def test_generate_result_explanations_uses_lite_model(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._ainvoke_provider_structured_chain", fake_invoke)
...
async def test_sample_data_tool_is_disabled_when_llm_data_samples_are_off(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.sample_data", fail_sample_data)
...
async def test_analysis_validate_result_skips_explanation_model_for_retryable_result(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._generate_result_explanations", fail_generate_result_explanations)
...
async def test_analysis_assess_context_falls_back_when_length_limit_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._ainvoke_structured_chain", fake_invoke)
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._get_model", lambda *_args, **_kwargs: _FakeModel())
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.ChatPromptTemplate.from_messages", lambda *_args, **_kwargs: _FakeChain())
