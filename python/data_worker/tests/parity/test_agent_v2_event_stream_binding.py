from inquira_data_worker.agent_v2.events import emit_agent_event
from inquira_data_worker.agent_v2.graph import _with_stream_event_emitter
from inquira_data_worker.agent_v2.nodes import chat_node, route_node
from inquira_data_worker.agent_v2.router import RouteDecision
...
async def test_route_node_emits_user_facing_reasoning(monkeypatch):
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.decide_route_details", _fake_decide_route_details)
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.emit_agent_event", lambda event, payload: captured.append((event, payload)))
...
async def test_route_node_prefers_workspace_table_description_before_router(monkeypatch):
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.decide_route_details", _should_not_call_router)
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes.emit_agent_event", lambda event, payload: captured.append((event, payload)))
...
async def test_chat_node_returns_table_clarification_without_model_call(monkeypatch):
    ...
    monkeypatch.setattr("inquira_data_worker.agent_v2.nodes._get_model", _should_not_get_model)
